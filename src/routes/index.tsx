          ...
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-7">
              ...
              <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-semibold tracking-tight text-primary leading-[1.05]">
                Know Your Value.<br />Plan Your Exit.<br /><span className="text-accent">Live Your Freedom.</span>
              </h1>
              ...
              <div className="relative rounded-2xl bg-gradient-to-br from-primary to-[oklch(0.28_0.07_250)] p-1 shadow-2xl">
                <div className="rounded-[14px] bg-card p-6">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="inline-flex items-center gap-1 rounded-full bg-accent-soft px-2 py-0.5 text-[10px] font-semibold text-accent"><TrendingUp className="h-3 w-3" /> +18%</span>
                  </div>
...
function Stat({ label, value, tone }: { label: string; value: string; tone: "warning" | "muted" }) {
  ...
  return <div className="rounded-lg border border-border bg-secondary/50 p-3"><div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground font-medium"><span className={`h-1.5 w-1.5 rounded-full ${dot}`} />{label}</div><div className="mt-1 text-sm font-semibold text-foreground">{value}</div></div>;
}
...
function Plan({ slug, name, price, sub, who, cta, features, highlighted }: { slug: string; name: string; price: string; sub: string; who: string; cta: string; features: string[]; highlighted?: boolean; }) {
  return <div className={`rounded-xl border p-6 ${highlighted ? "border-accent bg-card shadow-lg ring-2 ring-accent/20" : "border-border bg-card"}`}>
    {highlighted && <div className="inline-flex items-center rounded-full bg-accent px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-accent-foreground mb-3">Most popular</div>}
