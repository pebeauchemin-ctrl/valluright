-- REB-26: Secure OAuth token storage and add sensitive-action audit events.
--
-- Application code writes encrypted Xero tokens to xero_connections. This
-- migration prevents authenticated browser clients from reading token columns
-- directly, keeps the Data Room bucket private, and adds an owner-readable
-- audit table for sensitive events.
...
create policy "owners read own security audit events"
  ...
  using (
    ...
    or exists (
      ...
      where b.id = security_audit_events.business_id
