...
type Rec = {
  ...
  description: string; buyer_concern: string; action_steps: string[];
  estimated_impact_low: number; estimated_impact_high: number;
};
    ...
    <div className="p-6 lg:p-10 space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">AI-prioritized actions to grow what your business is worth.</p>
        </div>
        <button onClick={generate} disabled={loading}
          ...
          {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Generating…</> : <><Sparkles className="h-4 w-4" /> {recs.length ? "Regenerate" : "Generate with AI"}</>}
        </button>
      </div>
      ...
      {recs.length === 0 && !loading && (
        <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center">
          ...
          <h2 className="mt-3 font-display text-xl font-semibold text-primary">Ready to find hidden value?</h2>
          <p className="mt-2 text-sm text-muted-foreground max-w-md mx-auto">Click "Generate with AI" to get personalized actions that map directly to your valuation drivers.</p>
        </div>
      )}
          ...
          <div key={i} className="rounded-xl border border-border bg-card p-6">
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-3 min-w-0">
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${r.priority === "high" ? "bg-destructive/10 text-destructive" : r.priority === "medium" ? "bg-gold/15 text-foreground" : "bg-accent-soft text-accent"}`}>
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-xs italic text-muted-foreground"><strong>Buyer concern:</strong> {r.buyer_concern}</p>
                  {r.action_steps?.length > 0 && (
                    <ul className="mt-3 space-y-1 text-sm">
                      {r.action_steps.map((s, j) => <li key={j} className="flex gap-2"><ArrowRight className="h-3.5 w-3.5 mt-1 text-accent shrink-0" /><span>{s}</span></li>)}
                    </ul>
                  )}
                </div>
              </div>
              <div className="shrink-0 text-right">
                ...
                <div className="font-display text-lg font-semibold text-accent">+{fmtCurrency(r.estimated_impact_low, { compact: true })} – {fmtCurrency(r.estimated_impact_high, { compact: true })}</div>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="pt-4">
        <Link to="/app/scenarios" className="text-sm font-semibold text-accent hover:underline">Try a what-if scenario →</Link>
      </div>
    </div>
  );
}
