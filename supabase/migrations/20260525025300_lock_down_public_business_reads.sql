-- REB-43: Lock down unauthenticated reads from public.businesses.
--
-- The public teaser must not rely on anon access to the base businesses table.
-- This migration removes the broad anon SELECT policy and exposes only the
-- buyer-safe teaser payload through a security-definer RPC.
