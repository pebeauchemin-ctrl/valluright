-- REB-33: include the buyer-safe photo visibility flag in the public teaser RPC.
-- This does not expose files or image URLs; it only reflects the owner's saved
-- setting so the public teaser can show a safe availability prompt.
...
comment on function public.get_public_teaser(text) is
  'Intentional public SECURITY DEFINER RPC. Returns only owner-selected buyer-safe teaser fields.';
