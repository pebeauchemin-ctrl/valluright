...
const FIELD_KEYS = [
  "revenue", "cogs", "operating_expenses", "owner_salary", "addbacks",
  "depreciation", "amortization", "interest", "income_taxes", "net_income",
...
function Financials() {
  ...
  const runXeroImport = async () => {
    ...
    try {
      ...
      const { years: imported } = await importXero({ data: { tenantId: selectedTenant, years: requestedYears } });
  ...
  const handleQuickBooks = () => {
    toast.info("QuickBooks import coming soon. For now, export a CSV from QuickBooks and use the CSV import button.");
  };
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Pull P&amp;L and balance sheet data automatically, or upload a CSV. Imports merge into existing years; click Save to persist.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-md border border-border bg-background px-3 py-2 text-xs font-medium hover:bg-secondary"
            >
              <Link2 className="h-3.5 w-3.5" /> Import from QuickBooks
            </button>
          </div>
        </div>
