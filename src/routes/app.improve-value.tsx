...
const TEMPLATES: RecTemplate[] = [
  {
    ...
    description: "Delegate sales, operations, and customer relationships so the business runs without you. This is the single biggest multiple-mover for owner-operated SMBs.",
  ...
  },
  {
    ...
    description: "Get three full years of clean books, separate personal from business expenses, and build a clear add-back schedule. This raises confidence on every multiple.",
  ...
  },
  {
    ...
    description: "Diversify so no single customer is more than 15% of revenue. High concentration is the most common reason SMB deals fall apart in diligence.",
    ...
    <div className="p-6 lg:p-10 space-y-8">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Targeted moves to raise your valuation, ranked by impact for your business today.</p>
        </div>
        <div className="flex gap-2">
          <Link to="/app/health-score" className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-secondary">
            View health score{health ? ` · ${health.total}/100` : ""}
          </Link>
          <Link to="/app/recommendations" className="inline-flex items-center gap-1.5 rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground hover:bg-accent/90">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 text-sm text-muted-foreground">
        Today's estimated value: <span className="font-semibold text-foreground">{fmtCurrency(valuation.rangeLow, { compact: true })} – {fmtCurrency(valuation.rangeHigh, { compact: true })}</span>.
        Each card below estimates the lift on your <span className="font-semibold text-foreground">median</span> from acting on that lever. Combine multiple levers in <Link to="/app/scenarios" className="text-accent font-semibold hover:underline">What-if Scenarios</Link>.
      </div>
