create table if not exists public.import_sync_logs (
  ...
  retry_action text null check (
    retry_action is null
    or retry_action in ('xero_import', 'quickbooks_refresh', 'file_upload')
  ),
...
create policy "owners read own import sync logs"
  ...
  using (
    ...
    or exists (
      ...
      where b.id = import_sync_logs.business_id
...
comment on table public.import_sync_logs is
  'Safe import troubleshooting log. Do not store raw uploaded data, financial values, OAuth tokens, or buyer PII.';

comment on column public.import_sync_logs.metadata is
  'Safe metadata only, such as tenant or connection identifiers and row counts. Do not store imported financial values or file contents.';
