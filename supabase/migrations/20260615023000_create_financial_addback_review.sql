...
create policy "owners manage own financial addbacks"
  ...
  using (
    exists (
      ...
      where b.id = financial_addbacks.business_id
...
create policy "owners view own financial addback events"
  ...
  using (
    exists (
      select 1
      from public.businesses b
      where b.id = financial_addback_events.business_id
        and b.owner_id = auth.uid()
    )
  );
...
revoke all on table public.financial_addbacks from anon;
revoke all on table public.financial_addback_events from anon;
