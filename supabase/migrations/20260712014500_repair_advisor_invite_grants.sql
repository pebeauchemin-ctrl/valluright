-- REB-65: Repair advisor_invites table grants and owner/advisor policies.
--
-- Production drift left authenticated users with RLS policies but without the
-- table-level grant PostgREST needs before RLS can evaluate. Reapply the grants
-- and policies idempotently so owners can manage advisor invites again.
